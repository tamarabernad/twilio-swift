...
class ViewController: UIViewController, PKPushRegistryDelegate, TVONotificationDelegate, TVOIncomingCallDelegate, TVOOutgoingCallDelegate, CXProviderDelegate {
    let baseURLString = "http://0c63568a.ngrok.io"
    ...
    func fetchAccessToken() -> String? {
        ...
        guard let accessTokenURL = URL(string: self.baseURLString + self.accessTokenEndpoint + "?userId=" + userId) else {
            return nil
        }
    ...
    @IBAction func placeCall(_ sender: UIButton) {
        let uuid = UUID()
        let handle = self.tfReceiver.text!;
    ...
    func provider(_ provider: CXProvider, perform action: CXStartCallAction) {
        ...
        outgoingCall = VoiceClient.sharedInstance().call(accessToken, params: ["From":self.userId!,"To":self.tfReceiver.text!], delegate: self)
